  ...
  (lambda (f a)
    (let ((R (lambda (s) `(&const ,s)))
		  ...
		  ((eq? (car cl) `&const) (cadr cl))
                  ((is-thunk? cl) (check-thunk cl))
		  (else (error 'delta "non-constant argument"))))))
      (R (apply (R-1 f) (map R-1 a))))))

(define check-thunk
  (lambda (m) (if (eq? (car (force-thunk m)) '&const ) (cadr (force-thunk m)) (check-thunk (force-thunk m) ))))
...
(define ev
  (lambda (e env)
    (cond
      ((eq? (car e) '&const) e)
      ((eq? (car e) '&var) (if (eq? (car (lookup env (cadr e))) `&thunk) (force-thunk (lookup env (cadr e))) (lookup env (cadr e))))          ;; e == (&var v)
      ...
      ((eq? (car e) `&apply) (let ((f (cadr e))            ;; e == (&apply f args)
				   (args (caddr e)))
			       (let ((fv (ev f env))
				     (av (map (lambda (a) (if (check-exp? a) (mk-thunk env a) (ev a env))) args)))
